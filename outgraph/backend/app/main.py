...
async def global_exception_handler(request, exc):
    """Catches unhandled exceptions and returns structured JSON without exposing tracebacks."""
    logger.error(f"Unhandled exception on {request.url.path}: {str(exc)}")
    from fastapi.responses import JSONResponse
    return JSONResponse(
        status_code=500,
        content={
            "detail": "Internal scientific system error",
            "status": "ERROR",
            "safe_state": "UNKNOWN",
        },
    )
...
# Register API Routers with and without prefix for flexible integration
for r in ROUTERS:
