...
class EntityService:
    ...
    def resolve_entity(
        ...
        # Determine synthetic status from observation if not explicitly provided
        if is_synthetic is None:
            if observation_id:
                obs_rec = self.db.query(ObservationModel).filter(ObservationModel.id == observation_id).first()
                is_synthetic = obs_rec.is_synthetic if obs_rec else True
            else:
                is_synthetic = True
