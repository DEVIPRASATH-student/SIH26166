...
class PhysicsVerificationEngine:
    ...
    def verify_correspondence(
        ...
        src_res_val = src_meta.get("spatial_resolution_m") if src_meta else None
        tgt_res_val = tgt_meta.get("spatial_resolution_m") if tgt_meta else None
        src_res = float(src_res_val) if (src_res_val is not None and np.isfinite(src_res_val) and src_res_val > 0) else None
        tgt_res = float(tgt_res_val) if (tgt_res_val is not None and np.isfinite(tgt_res_val) and tgt_res_val > 0) else None
