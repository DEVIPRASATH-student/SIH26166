...
class ScaleSpatialVerifier:
    ...
    def verify(
        ...
        src_res_m: Optional[float] = None,
        tgt_res_m: Optional[float] = None,
        ...
        n = len(pts1)
        has_valid_gsd = (
            src_res_m is not None
            and tgt_res_m is not None
            and np.isfinite(src_res_m)
            and np.isfinite(tgt_res_m)
            and src_res_m > 0
            and tgt_res_m > 0
        )
        expected_scale = float(src_res_m / max(tgt_res_m, 1e-4)) if has_valid_gsd else 1.0
        ...
        if has_valid_gsd:
            ...
            scale_valid = scale_score >= 0.25
        else:
            scale_score = 0.5  # Neutral UNKNOWN scale confidence
            scale_valid = True
        ...
        is_valid = (avg_hull_ratio >= self.min_spatial_coverage) and scale_valid

        reason = (
            f"Scale ({estimated_scale:.2f}x) and spatial dispersion verified (Coverage={avg_hull_ratio*100:.1f}%, Entropy={norm_entropy:.2f})"
            if is_valid and has_valid_gsd
            else (
                f"Spatial dispersion verified with UNKNOWN GSD (Coverage={avg_hull_ratio*100:.1f}%, Entropy={norm_entropy:.2f})"
                if is_valid
